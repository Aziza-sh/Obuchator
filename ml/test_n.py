...
class RobustThesisExtractor:
    ...
    def _init_backup_keywords(self):
        ...
        self.backup_keywords = {
            'искусственный интеллект': 4.0, 'ии': 3.5, 'машинное обучение': 4.0,
            'глубокое обучение': 3.5, 'нейронная сеть': 3.0, 'алгоритм': 2.5,
            'данные': 2.0, 'модель': 2.0, 'обучение': 2.5, 'прогнозирование': 2.0,
            'анализ': 2.0, 'обработка': 2.5, 'nlp': 3.0, 'компьютерное зрение': 2.5,
            'этика': 2.5, 'безопасность': 2.0, 'применение': 2.0, 'будущее': 1.5,
            'развитие': 1.5, 'технология': 2.0, 'система': 2.0,
            'сеть': 2.0, 'программа': 1.5, 'код': 1.5, 'разработка': 2.0,
            'инновация': 2.0, 'цифровой': 2.0, 'автоматизация': 2.5,
            'робот': 2.0, 'бот': 1.5, 'чат': 1.5, 'ассистент': 1.5,
            'платформа': 1.5, 'сервис': 1.5, 'приложение': 1.5
        }
    ...
    def extract_theses(self, text: str, num_theses: int = 7) -> List[str]:
        ...
        try:
            ...
            if len(result_sentences) < num_theses:
                additional = self._get_best_sentences(key_sentences, num_theses - len(result_sentences))
    ...
    def _score_sentence(self, sentence: str, position: int, total: int) -> float:
        ...
        # Наличие определений
        if any(word in s_lower for word in ['это', 'означает', 'определение', 'является']):
            score += 2.0
        
        # Наличие конкретики
        if any(word in s_lower for word in ['например', 'в частности', 'таких как', 'следующий']):
            score += 1.5
    ...
    def _generate_summary(self, text: str, num_sentences: int) -> str:
        ...
        try:
            ...
            if word_count < 200:
                params['max_length'] = min(params['max_length'], max(60, word_count // 2))
                params['min_length'] = min(params['min_length'], max(30, word_count // 4))
...
def analyze_theses(theses: List[str], original_text: str = None):
    ...
    quality_checks = {
        ...
        "Нет обрывков": not any(t.strip().endswith(('котор', 'что', 'для', 'на', 'в', 'с', 'и', 'а', 'но')) for t in theses),
...
def check_themes_coverage(theses: List[str], original_text: str) -> Dict[str, bool]:
    ...
    themes = {
        "Определение ИИ": ["искусственный интеллект", "ии ", " ai ", "artificial intelligence"],
        ...
        "Обработка языка": ["естественный язык", "nlp", "обработка текста", "natural language"],
        "Применение ИИ": ["применение", "использование", "application", "используется"],
        "Этика и риски": ["этика", "риск", "безопасность", "ответственность", "этический"]
    }
